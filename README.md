...
　　・ログイン中名の修正
　　・記録一覧のボタンを上に
　　・長時間後は、ログアウトor　画面そのままに
　（３）音楽再生画面
　　・７/３夜　不具合対応
　　　ブラウザ実行すると、初期画面で、また音楽ファイルがみつかりません　　　の状態に戻ってしまいました・・。
　　　・9～15時調整するもNG
　　　・再度、原因の推定をして、対応を行った　
　　　　⇒朝、ここで確認が甘かったのが、6時間のロスに？
　　　　　＊部分最適の提案の可能性あり、提案内容をしっかり理解して、進める必要あり
　　　　対応内容
　　　　・音楽再生開始時に「55分後に自動でトークンリフレッシュAPI」を呼ぶ仕組みの実装
　　　　・サーバー側でリフレッシュトークンを使い、アクセストークンを安全に更新します。
　　　　・失敗時のみユーザーに再ログインを促す
　　　　⇒再生後だけでなく、HP開始後も追加
　　　　＊動くかテスト必要！！
...
＜Google cloud リフレッシュトークン　デバッグ削除内容＞
